...
function questionPage() {
  if (currentQuestionIndex < words()[0].length) {
    document.getElementById("question").innerText =
      words()[0][currentQuestionIndex];
    document.getElementById("hint").innerText =
      words()[2][currentQuestionIndex];
...
function words() {
  ...
  let hint = [
    "mempunyai tangkai dan berakar",
    "Biasa digunakan untuk tidur",
    "sering digunakan untuk membantu pengelihatan",
    "alat untuk mengeringkan badan dari air",
    "sepasang dan digunakan untuk melindungi kaki",
    "digunakan untuk menerangi ruangan",
    "wadah untuk menampung air minum",
    "alat untuk menulis dan dapat dihapus",
...
function leaderboard() {
  ...
  let sortedProducts = dummy.sort((p1, p2) =>
    p1.totalScore < p2.totalScore ? 1 : p1.totalScore > p2.totalScore ? -1 : 0
  );
  ...
  sortedProducts.forEach((element, index) => {
    if (index !== 0) {
      ...
      document.getElementById(
        "top1-winner"
      ).innerHTML = `${element["name"]}: ${element["totalScore"]}`;
    }
  });
